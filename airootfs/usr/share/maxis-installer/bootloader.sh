...
detect_other_os() {
    ...
    if [ "$BOOT_MODE" = "uefi" ] && [ -n "$ESP_PATH" ] && [ -d "$ESP_PATH/EFI" ]; then
        ...
        for dir in "$ESP_PATH/EFI"/*; do
            ...
            case "$dirname" in
                systemd|BOOT|Boot|refind|tools|Maxis)
    ...
    if [ "$BOOT_MODE" = "uefi" ] && command -v efibootmgr &>/dev/null; then
        ...
        if echo "$efi_output" | grep -qiE "Windows|Microsoft|Ubuntu|Manjaro|Debian|Fedora|openSUSE|Mint"; then
            local non_maxis_entries=$(echo "$efi_output" | grep -iE "Windows|Microsoft|Ubuntu|Manjaro|Debian|Fedora|openSUSE|Mint" | grep -v -i "Maxis")
            if [ -n "$non_maxis_entries" ]; then
                local found_entry=$(echo "$non_maxis_entries" | head -1)
...
EOF
    ...
    cat > "$esp_path/loader/entries/maxis.conf" <<EOF
title   Maxis
linux   /vmlinuz-linux
...
EOF
    ...
    cat > "$esp_path/loader/entries/maxis-fallback.conf" <<EOF
title   Maxis (fallback)
linux   /vmlinuz-linux
...
EOF
    ...
    if [ -n "$esp_device" ]; then
        ...
        if [ -n "$esp_part_num" ] && [ -n "$esp_disk" ]; then
            efibootmgr -c -d "$esp_disk" -p "$esp_part_num" -L "Maxis" -l "\\EFI\\systemd\\systemd-bootx64.efi" 2>/dev/null || true
            
            local linux_boot=$(efibootmgr | grep "Maxis" | grep -o '^Boot[0-9A-F]\{4\}' | sed 's/Boot//' | head -1)
...
install_grub() {
    ...
    if [ "$boot_mode" = "uefi" ]; then
        ...
        if ! grub-install --target=x86_64-efi --efi-directory="$esp_path" --bootloader-id=Maxis; then
...
EOF
    ...
    sed -e 's/Arch Linux/Maxis/g' \
        -e "s/menuentry 'Arch/menuentry 'Maxis/g" \
        -e 's/menuentry "Arch/menuentry "Maxis/g' \
...
"Boot Maxis"                 "root=UUID=$root_uuid rw quiet splash"
"Boot Maxis (terminal)"      "root=UUID=$root_uuid rw systemd.unit=multi-user.target"
