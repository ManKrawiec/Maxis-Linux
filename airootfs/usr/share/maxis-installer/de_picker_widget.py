...
class DEPicker(Gtk.Box):
    ...
    def __init__(self, on_continue_callback=None, **kwargs):
        ...
        self.options = [
            ...
            {
                "name": "Hyprland",
                "description": "Hyprland (Wayland) + Kitty",
                "icon": "screen3.png",
                "requires_internet": True
            }
        ]
        ...
        # Info label for Hyprland setup instructions
        hypr_info = Gtk.Label()
        hypr_info.set_text(_("Hyprland setup instructions will be available on your Desktop after installation."))
        hypr_info.set_halign(Gtk.Align.CENTER)
        hypr_info.set_wrap(True)
        hypr_info.set_justify(Gtk.Justification.CENTER)
        hypr_info.set_margin_top(10)
        hypr_info.add_css_class("option_details")
        self.append(hypr_info)
        ...
        # Flatpak checkbox
        self.flatpak_check = Gtk.CheckButton(label="Download Maxis-suggested Flatpaks")
    ...
    def create_option_box(self, option, index, script_dir):
        ...
        if not icon_loaded:
            ...
            fallback_icons = ["📦", "💼", "🪟"]
            fallback_label.set_text(fallback_icons[index] if index < len(fallback_icons) else "📦")
